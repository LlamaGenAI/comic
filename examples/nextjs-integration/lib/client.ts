...
export interface ServerEnv {
  ...
  WEBTOON_API_KEY?: string;
}

export function createServerLlamaGenClient(env: ServerEnv = process.env): LlamaGenClient {
  const apiKey = env.LLAMAGEN_API_KEY ?? env.WEBTOON_API_KEY;
  if (!apiKey) {
    throw new Error('Missing LLAMAGEN_API_KEY (or WEBTOON_API_KEY) for Next.js integration demo.');
  }
